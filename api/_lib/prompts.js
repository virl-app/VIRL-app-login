...
const PLATFORM_COMPOSER_BLOCK = ""
  ...
  + " `pinDescription`: 2-4 sentences, 150-500 characters, written as sentences a person would actually read — Pinterest ranks this as natural-language text, not as a tag list. Carry 2-3 DISTINCT keyword phrases: the primary one INSIDE THE FIRST SENTENCE, because feed and search views truncate the description hard and a phrase in sentence four may as well not be there, and the other one or two woven through the sentences after it. Vary the wording between them instead of repeating one phrase — the same string three times is stuffing, and it costs the two extra queries the pin could have ranked for. Say what the pin delivers, then what happens at the link. Never a rephrasing of `pinTitle`."
  ...
  + " PINTEREST KEYWORD CLUSTER: `pinTitle`, `pinDescription`, `pinBoard` and `pinTopics` are matched together, so all four must be about ONE thing. The primary phrase, or a close variant of it, belongs in every one of them. A pin titled for one query, described for a second and filed on a board about a third splits its own relevance three ways and ranks for none of them."
  ...
  + " SEARCH KEYWORDS — SOURCE AND HONESTY (Pinterest and YouTube only). Both are search engines before they are feeds, so every card on them is answering a query somebody typed. Build that phrase from what THIS PROMPT actually gives you: the creator's niche detail, their audience, the problem their ideal client describes, the offerings they named, their service area, and the card's own subject. Prefer long-tail, high-intent phrasing ('content strategy for small business owners') over a broad head term ('marketing', 'business tips') — the broad term is where a small account never surfaces, and the long-tail one is closer to what a real person types anyway."
  + " You have NO search-volume or keyword-difficulty data in this prompt, for either platform. NEVER state or imply that a term is trending, high-volume, low-competition, or researched, in any field the creator reads — pick the phrase someone in this audience would plausibly type, and say nothing about its numbers. The trends block above is about content trends and does not label which platform each item came from, so it is not a source of Pinterest or YouTube search terms either."
...
function buildPlan(params, profile, vaultPatterns, playbook, trends, history, recentEdits, compliance, personalDenylist) {
  ...
  const sharedSystemPrompt = VIRL_PERSONA + "\n\n"
    ...
    + " format=video → include `hook` (1-2 sentence opening for the first 1.5 seconds), `caption` (the post caption), `onScreenText` (array of suggested text overlays during the video, 2-5 items), `audioRecommendation` (style of audio/sound to use), `filmDirection` (2-3 concrete clips to capture, as one string separated by semicolons — each specific enough to shoot from: subject + location + action + framing, e.g. 'slow walkthrough of the kitchen from the doorway, phone at chest height'. Phone-shootable only, no crew or equipment assumptions, and at least one option that works without the creator's face on camera (b-roll + voiceover) so camera-shy days still ship). Keep `description` strategic — the filming instructions live in `filmDirection`, not the description."
    + " format=single_image → include `caption` (the post caption — carries the post), `photoDirection` (concrete enough to shoot from: subject + location + light + angle, phone-shootable — e.g. 'front porch at golden hour, shot from the walkway, door slightly open'. When an existing photo would work just as well, say so: 'or pull from your camera roll: any recent [X] shot — run it through Scan to check'), `compositionTip` (one specific tip for making the photo land)."
