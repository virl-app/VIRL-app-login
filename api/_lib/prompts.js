...
function buildPlan(params, profile, vaultPatterns, playbook, trends, history, recentEdits, compliance, personalDenylist) {
  ...
  const sharedSystemPrompt = "You are VIRL, an AI content strategist and creative director. "
    ...
    + " format=video → include `hook` (1-2 sentence opening for the first 1.5 seconds), `caption` (the post caption), `onScreenText` (array of suggested text overlays during the video, 2-5 items), `audioRecommendation` (style of audio/sound to use). ALSO: end the card's `description` with 'Film: ' followed by 2-3 concrete clips to capture — each one specific enough to shoot from (subject + location + action + framing, e.g. 'slow walkthrough of the kitchen from the doorway, phone at chest height'). Phone-shootable only, no crew or equipment assumptions, and at least one option that works without the creator's face on camera (b-roll + voiceover) so camera-shy days still ship."
